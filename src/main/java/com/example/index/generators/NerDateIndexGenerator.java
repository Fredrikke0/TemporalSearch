...
public final class NerDateIndexGenerator extends IndexGenerator<AnnotationEntry> {
    ...
    public NerDateIndexGenerator(IndexAccessInterface indexAccess, String stopwordsPath, Connection sqliteConn,
    ...
    protected ListMultimap<IndexKey, PostingList> processBatch(List<AnnotationEntry> batch) {
        ...
        // We just need to sort the batch by position for the merging logic to work
        // correctly.
    ...
    public static String normalizeDateToKeyFormat(String date) {
        ...
        try {
            ...
            try {
                ...
                // LocalDate.parse with "yyyy-MM" pattern will parse "YYYY-MM" to the first day
                // of that month.
                ...
                try {
                    ...
                    // However, to be more explicit or handle cases where direct parsing "yyyy"
                    // isn't standard for LocalDate:
                    ...
                    logger.trace("Could not parse date string '{}' with available formats (YYYY-MM-DD, YYYY-MM, YYYY).",
                            trimmedDate);
                    return null;
                }
            }
