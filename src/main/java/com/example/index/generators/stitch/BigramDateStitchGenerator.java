...
public final class BigramDateStitchGenerator extends AbstractNgramStitchGenerator {
    ...
    protected List<AnnotationData> fetchAnnotationsForDocument(int documentId) throws SQLException {
        ...
        List<AnnotationData> filteredAnnotations = filterAnnotationsBySentenceCharacterSpan(
            rawAnnotationsListFromDb, documentId, "Bigram Date");

        if (filteredAnnotations.isEmpty()) {
            if (!rawAnnotationsListFromDb.isEmpty()) {
                logger.trace("No valid DATE annotations remaining after span filtering for document ID {} for {} (Bigram) index.", documentId, MY_INDEX_NAME);
            } else {
        ...
        if (mergedAnnotations.isEmpty() && !filteredAnnotations.isEmpty()) {
            logger.trace("All filtered DATE annotations for document ID {} resulted in an empty merged list for {} (Bigram) index. Filtered count: {}", documentId, MY_INDEX_NAME, filteredAnnotations.size());
        } else {
