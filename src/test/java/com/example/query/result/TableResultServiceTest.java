...
class TableResultServiceTest {
    ...
    void testGenerateTableForJoinDocumentGranularity() throws ResultGenerationException {
        ...
        // Expect only the default ID columns
        List<String> expectedColumns = List.of("left_document_id", "right_document_id");
        assertEquals(expectedColumns, table.columnNames(), "Should only contain default ID columns when no SELECT is specified");
        
        // Verify ID values
        assertEquals(1, table.intColumn("left_document_id").get(0));
        assertEquals(2, table.intColumn("right_document_id").get(0));
        assertEquals(3, table.intColumn("left_document_id").get(1));
        assertEquals(4, table.intColumn("right_document_id").get(1));
        
        // Assertions for main.fruitL and main.fruitR are removed as they shouldn't be generated by default
    }
